...
/* LongBM: By Default maxSockets = 5. We need to incre to 200 */
...
FetchStream.prototype.parseUrl = function(url){
    var urlparts = urllib.parse(url, false, true),
        transport,
        urloptions = {
            host: this.options.hostname || urlparts.hostname || urlparts.host,
            port: this.options.port || urlparts.port,
    ...
    /**
	 |-----------------------------------------------------------------
     | Full url if using Proxy
     |-----------------------------------------------------------------
	 */
	if( this.options.hostname 
        && this.options.hostname !== (urlparts.hostname || urlparts.host) )
	{
		urloptions.path = url;
	}
	else
	{
		urloptions.path = urlparts.path;
	}
